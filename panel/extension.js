...
const BAR_COINS = new Set(['BTC', 'XRP']);
...
class CoinwatchIndicator extends PanelMenu.Button {
    ...
    _refresh() {
        const ids = WATCHLIST.map(c => c[0]).join(',');
    ...
    _render(data) {
        ...
        for (const [id, decimals, label] of WATCHLIST) {
            const entry = data[id];
            ...
            const item = new PopupMenu.PopupMenuItem(`${label}  $${priceFmt}  ${arrow}${changeFmt}%`);
            item.connect('activate', () => {
                Gio.AppInfo.launch_default_for_uri(`https://www.coingecko.com/en/coins/${id}`, null);
            });
            this.menu.addMenuItem(item);

            if (BAR_COINS.has(label)) {
                const sign = change > 0 ? '+' : change < 0 ? '-' : '';
                barParts.push(`${label} <span foreground='${color}'>$${priceFmt} ${sign}${changeFmt}%</span>`);
            }
        }
